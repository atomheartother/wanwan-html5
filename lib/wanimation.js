...
// The parameter is the animation the function was queued for
var animHandler = function () {
    ...
    if (aIndex >= currentAnimation.steps.length) {
        ...
        // Never touch the index again until it's set by an external source
        aIndex = -1;
    }
};
...
document.addEventListener('visibilitychange', function () {
    if (!document.hidden) {
        var time = 0;
        for (var index in currentAnimation.steps) {
            var n = currentAnimation.steps[index];
